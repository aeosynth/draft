var EventEmitter, Bot, Human, db, _, genPack, Draft;
...
Draft = (function(superclass){
  ...
  prototype.add = function(p){
    ...
    p.on('meta', bind$(this, 'meta'));
    p.on('drop', bind$(this, 'drop'));
    if (p.isHost) {
      p.once('start', bind$(this, 'start'));
    }
    return this.meta();
  };
  prototype.drop = function(p){
    var index;
    if (this.round) {
      return;
    }
  ...
  prototype.startRound = function(){
    var r, set, i$, ref$, len$, p, results$ = [];
    r = ++this.round;
    this.send('set', {
      round: r
    });
    if (!(set = this.sets[r - 1])) {
      return this.end();
    }
    this.delta *= -1;
    ...
    for (i$ = 0, len$ = (ref$ = this.players).length; i$ < len$; ++i$) {
      p = ref$[i$];
      p.startRound();
      results$.push(p.receive(genPack(set)));
    }
    return results$;
  };

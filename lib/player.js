...
Player.prototype.send = function msg(name, args) {
...
Player.prototype.receive = function(pack) {
  if (this.packs.push(pack) === 1) {
    if (this.dropped)
      // wait until all new packs have been opened before picking
      return process.nextTick(this.autoPick);
    this.time = this.TIME;
    this.updateSelf();
  }
};

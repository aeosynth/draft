...
function Player(ws, id, name, TIME, isBot) {
  this.isBot = isBot;
  this.id = id;
  this.name = name;
  this.packs = [];
...
Player.prototype.pick = function(packId, name) {
  this.hasAutoPicked = false;
  this.pickCard(packId, name);
};
...
Player.prototype.autoPick = function() {
  ...
  var packs = this.packs
    , pack = packs[0]
    , packId = pack.id
    , name = _.shuffle(pack.cards)[0].name
    ;
  this.pickCard(packId, name);
...
Player.prototype.pickCard = function(packId, name) {
  var packs = this.packs
    , pack = packs[0]
    , cards
    , card
  ...
  cards = pack.cards;
  card = _.find(cards, function(card) { return card.name === name; });
